...
export function useStripeCheckout(options?: UseStripeCheckoutOptions) {
  ...
  const initiateCheckout = useCallback(
    async (params: {
      ...
      try {
        ...
        const redirectUrl =
          result.order.authorization_url || result.order.authorizationUrl;

        // Redirect to external Stripe / Gateway checkout if URL provided
        if (redirectUrl && !result.order.clientSecret) {
  ...
  /**
   * Confirms payment reference after gateway redirect
